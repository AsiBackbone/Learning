...
> **Section status:** Focused ASP.NET Core learning is expanding. Start with [Middleware Ordering Changes Behavior](middleware-ordering-changes-behavior.md), continue with [Secure-by-Default ASP.NET Core Configuration](secure-by-default-configuration.md), then read [Structured Logging Without Sensitive-Data Sprawl](structured-logging-without-sensitive-data-sprawl.md), [Centralized Error Handling and Problem Details](centralized-error-handling-and-problem-details.md), [Data-Access Boundaries and Transaction Reasoning with EF Core](data-access-boundaries-and-transaction-reasoning.md), [Architecture Decision Records Preserve Architectural Reasoning](architecture-decision-records-preserve-architectural-reasoning.md), [Architecture Decision Record Lifecycle, Review, Deprecation, and Supersession](architecture-decision-record-lifecycle-review-deprecation-and-supersession.md), and [Working Repository ADR Case Study: NetCoreApplicationTemplate](netcoreapplicationtemplate-adr-case-study.md), then complete the [Write and Revisit an Architecture Decision Record lab](../labs/write-and-revisit-an-architecture-decision-record.md). Use the [Foundational Tutorials](../tutorials/index.md) when you want to connect application structure to governed execution.
...
[Working Repository ADR Case Study: NetCoreApplicationTemplate](netcoreapplicationtemplate-adr-case-study.md) then follows two accepted decisions from general Learning principles into repository-specific constraints, ADR rationale, concrete code and configuration, alternatives, consequences, and review triggers. It uses the working repository as a specimen rather than a universal standard.

Complete the [Write and Revisit an Architecture Decision Record lab](../labs/write-and-revisit-an-architecture-decision-record.md) to make a logging-related decision yourself, preserve its alternatives and consequences, define evidence-based review conditions, and then choose the correct lifecycle response after the scenario changes.
...
- [Hands-on ADR writing and lifecycle review](../labs/write-and-revisit-an-architecture-decision-record.md)
